...
public class JdbcURL {
    ...
    public static JdbcURL of(@NonNull String url) throws SQLException {
        ...
        try {
            ...
            throw new SQLException("Invalid URI syntax: " + e.getReason(), e);
        }

        if (!uriObj.getPath().equals("")) {
            throw new SQLException("JDBC URLs must not contain a path");
        }
        if (uriObj.getFragment() != null) {
            throw new SQLException("JDBC URLs must not contain a fragment");
        }
        if (uriObj.getUserInfo() != null) {
            throw new SQLException("JDBC URLs must not contain a user info");
        }
        if (uriObj.getHost() == null || uriObj.getHost().isEmpty()) {
            throw new SQLException("JDBC URLs must contain a host");
        }
     ...
     */
    private static Map<String, String> parseQueryString(String query) throws SQLException {
        ...
        for (String pair : pairs) {
            ...
            if (keyValue.length == 1) {
                throw new SQLException("Parameter without value in JDBC URL: " + keyValue[0], "HY000");
            }
            ...
            if (parameters.containsKey(key)) {
                throw new SQLException("Duplicate parameter key in JDBC URL: " + key, "HY000");
            }
     ...
     */
    public void addParametersToProperties(Properties properties) throws SQLException {
